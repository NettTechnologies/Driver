...
*/
void
j1939tp_rtscts_failed ( uint8_t index ) {
...
*/
uint8_t
j1939tp_rxbuf_find ( uint8_t dst, uint8_t src, uint32_t pgn ) {
...
*/
uint8_t
j1939tp_txbuf_find ( uint8_t dst, uint8_t src, uint32_t pgn ) {

  uint8_t cnt;

  for( cnt = 0; cnt < J1939CFG_TP_TX_BUF_NUM; cnt++ )
    if( (j1939tp_txbuf[cnt].state != J1939TP_STATE_UNUSED)
         && (j1939tp_txbuf[cnt].dst == src)
         && (j1939tp_txbuf[cnt].src == dst)
         && (j1939tp_txbuf[cnt].pgn == pgn) )
      break;
...
*/
uint8_t
j1939tp_tx_bam_rts ( uint8_t index, uint8_t ctrl_byte ) {
...
*/
uint8_t
j1939tp_tx_abort ( uint8_t dst, uint8_t rsn, uint32_t pgn ) {
...
*/
void
j1939tp_state_rx_bam ( uint8_t index, j1939_t *msg ) {
  ...
  switch( j1939tp_rxbuf[index].state ) {
    ...
    case J1939TP_STATE_WAIT_DATA: {
      ...
      if( (j1939tp_rxbuf[index].next_seq == msg->buf[0])
        ...
        if( j1939tp_rxbuf[index].next_seq++ ==
            j1939tp_rxbuf[index].num_packets ) {
...
*/
void
j1939tp_update_rx_bam ( uint8_t index ) {
...
*/
void
j1939tp_update_tx_bam ( uint8_t index ) {
...
*/
void
j1939tp_state_rx_rtscts ( uint8_t index, j1939_t *msg ) {
...
*/
void
j1939tp_update_rx_rtscts( uint8_t index ) {
  ...
  switch( j1939tp_rxbuf[index].state ) {
    ...
    case J1939TP_STATE_SEND_ABORT: {
      if( j1939tp_tx_abort(j1939tp_rxbuf[index].src,
                           j1939tp_rxbuf[index].rsn,
...
*/
void
j1939tp_update_tx_rtscts( uint8_t index ) {
  ...
  switch( j1939tp_txbuf[index].state ) {
    ...
    case J1939TP_STATE_SEND_ABORT: {
      ...
      if( j1939tp_txbuf[index].cts_rcvd ) {
        ...
        if( j1939tp_tx_abort(j1939tp_txbuf[index].dst,
                             j1939tp_txbuf[index].rsn,
